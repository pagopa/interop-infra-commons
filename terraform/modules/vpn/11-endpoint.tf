...
resource "aws_security_group" "vpn" {
  ...
  vpc_id      = var.vpc_id
  tags        = merge(var.tags, { Name = local.sg_tag_name })
}
...
resource "aws_cloudwatch_log_group" "vpn" {
  ...
  tags              = merge(var.tags, { Name = local.cw_tag_name })
}
...
resource "aws_ec2_client_vpn_endpoint" "this" {
  description            = local.endpoint_desc
  ...
  tags = merge(var.tags, { Name = local.endpoint_tag })
}
...
resource "aws_ec2_client_vpn_authorization_rule" "mutual_cert" {
  ...
  client_vpn_endpoint_id = aws_ec2_client_vpn_endpoint.this.id
  target_network_cidr    = local.authz_cidr
  authorize_all_groups   = true
  description            = local.authz_desc
}
