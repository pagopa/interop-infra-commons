#Issue aperto sul provider TLS:
#  https://github.com/hashicorp/terraform-provider-tls/issues/728

# common

# repro ephemeral:

# ephemeral "tls_private_key" "server_ca" {
#   algorithm = "RSA"
#   rsa_bits  = 2048
# }
...
resource "aws_acm_certificate" "vpn_server" {
  count = local.create_server_pki ? 1 : 0
  # repro ephemeral:
  # private_key = ephemeral.tls_private_key.server.private_key_pem
...
# chiave in state TF (sensitive) - la copia in SM usa secret_string_wo, ma tls_locally_signed_cert richiede la chiave in chiaro
# Creata solo quando la PKI client è interna.
...
resource "tls_locally_signed_cert" "client_admin" {
  ...
  # repro ephemeral:
  # ca_private_key_pem = ephemeral.tls_private_key.client_ca.private_key_pem
  ca_private_key_pem    = tls_private_key.client_ca[0].private_key_pem
  ca_cert_pem           = tls_self_signed_cert.client_ca[0].cert_pem
  validity_period_hours = var.cert_validity_hours
