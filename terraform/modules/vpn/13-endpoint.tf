...
resource "aws_security_group" "vpn" {
  ...
  vpc_id      = var.vpc_id
  tags        = merge(var.tags, { Name = "${local.endpoint_name_prefix}-vpn-sg" })
}
...
resource "aws_cloudwatch_log_group" "vpn" {
  ...
  tags              = merge(var.tags, { Name = "${local.endpoint_name_prefix}-vpn-logs" })
}
...
resource "aws_ec2_client_vpn_endpoint" "this" {
  ...
  tags = merge(var.tags, { Name = "${local.endpoint_name_prefix}-vpn-endpoint" })
}

resource "aws_ec2_client_vpn_network_association" "this" {
  for_each               = local.subnet_ids_by_index
...
resource "aws_ec2_client_vpn_authorization_rule" "mutual_cert" {
  ...
  description            = var.mutual_cert_authorization_rule_description
}
...
resource "aws_ec2_client_vpn_authorization_rule" "saml" {
  ...
  description            = local.resolved_saml_authorization_rule_desc
}
